<?php
/**
 * CodegaUpdater - Admin Area View
 *
 * Available variables:
 *   $link, $dir_link, $dir_path, $dir_name, $name, $version, $fields, $settings
 */
...
<style>
.cu-shell { font-family: -apple-system, "Segoe UI", sans-serif; max-width: 1100px; margin: 0 auto; padding: 12px 0 32px; }
...
<div class="cu-shell">
    ...
    <div class="cu-grid">
        <div class="cu-card">
            ...
            <ul class="cu-info">
                ...
                <li><span class="cu-info-l">Tema Yolu</span><span class="cu-info-v"><code><?php echo htmlspecialchars($theme_path, ENT_QUOTES); ?></code></span></li>
            </ul>
        </div>
    ...
    <div class="cu-alert cu-alert-info">
        <div>
            <strong>Nasil Calisir?</strong> Bu eklenti, Codega temasinin GitHub deposundaki en son release'i kontrol eder. Yeni surum bulunursa ZIP olarak indirip otomatik olarak <code><?php echo htmlspecialchars($theme_path, ENT_QUOTES); ?></code> klasorune uygular. <code>config.php</code> ve <code>theme-config.php</code> dosyalari korunur.
        </div>
    </div>
...
(function(){
    ...
    function call(action, cb) {
        var url = ajaxBase + '/' + action + '?_t=' + Date.now();
        fetch(url, { credentials: 'same-origin' })
            .then(function(r){ return r.text(); })
            .then(function(html){
                // Response body'sinde <pre id="cdg-update-response"> var
                var match = html.match(/<pre id="cdg-update-response"[^>]*>([^<]+)<\/pre>/);
                if(match) {
                    try {
                        var jsonStr = match[1].replace(/&quot;/g,'"').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>');
                        cb(JSON.parse(jsonStr));
                    } catch(e) {
                        cb({status:'error', message: 'JSON parse hatasi: ' + e.message + ' | Raw: ' + match[1].substring(0,200)});
                    }
                } else {
                    cb({status:'error', message:'Beklenmeyen cevap formati. URL: ' + url});
                }
            })
    ...
    function renderUpdate(data) {
        if(data.status !== 'successful') {
            resultBox.innerHTML = '<div class="cu-alert cu-alert-danger"><div><strong>Hata:</strong> ' +
                (data.message || 'Bilinmeyen hata') + '</div></div>';
        ...
        var html = '';
        if(data.has_update) {
            html += '<div class="cu-alert cu-alert-success"><div>' +
            ...
            html += '<div class="cu-alert cu-alert-success"><div>' +
                    '<strong>Tema en guncel surumde!</strong><br>Yuklu: v' + data.current_version + '</div></div>';
        }
                ...
                call('apply', function(r){
                    if(r.status === 'successful') {
                        resultBox.innerHTML = '<div class="cu-alert cu-alert-success">' +
                            '<div><strong>' + (r.message || 'Guncelleme basarili!') + '</strong><br>Sayfa yeniden yuklenecek.</div></div>';
                        setTimeout(function(){ window.location.reload(); }, 2000);
                    } else {
                        resultBox.innerHTML = '<div class="cu-alert cu-alert-danger">' +
                            '<div><strong>Guncelleme basarisiz:</strong><br>' + (r.message || 'Bilinmeyen hata') + '</div></div>';
